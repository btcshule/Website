...
class Login extends CI_Controller {
  ...
  public function check_login(){
    ...
    if (!empty($get)) {
      ...
      if (!empty($get)) {
        $sess_data =array();
          $sess_data = array('EMPLOYE_ID'=>$get['EMPLOYE_ID'],'EMAIL_EMP'=>$get['EMAIL_EMP'],'EMPLOYE'=>$get['EMPLOYE'],'PROFILE_ID'=>$get['PROFILE_ID'],'DESC_PROFIL'=>$get['DESC_PROFIL'],'IS_USER_SYSTEM'=>$get['IS_USER_SYSTEM'],'IS_MUST_CHANGE_PWD'=>$get['IS_MUST_CHANGE_PWD'],'ID_BRANCHE'=>$get['ID_BRANCHE']
        );


          $this->session->set_userdata($sess_data);
        
        $output = array("status"=>TRUE,'message'=>'Traitement en cours...');
      }else{
        $output = array("status"=>FALSE,'message'=>'Le mot de passe ou email  incorrect.');
      }
    }else{
  ...
  function go_submit()
  {
    ...
    redirect(base_url('index.php/rapport/Dashboard')); 
  }
  ...
  function valid_pwd_change()
  {
    ...
    $this->Model->update('employes',array('EMAIL'=>trim($EMAIL)),array('IS_MUST_CHANGE_PWD'=>0,'MOT_DE_PASSE'=>md5($CONFIRM_PWD)));
  ...
  public function _validate()
  {
    ...
    if($pseudo == '')
    {
      $data = array('status'=>FALSE,'message'=>'Le nom utilisateur est obligatoire');
      $stat = false;
    }else{
      if($pwd == '')
      {
        ...
        $get = $this->Model->getOne('employes',array('EMAIL_EMP'=>$this->input->post('EMAIL')));
