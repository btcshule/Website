...
class Clients extends CI_Controller
{
	...
	{    
		...
		// SELECT `GROS_CLIENT_ID`, `NOM_GROS_CLIENT`, `ADRESS_GROS_CLIENT`, `TEL_GROS_CLIENT`, `IS_CLIENT`, `NIF`, `RS`, `RC`, `ID_BRANCHE` FROM `gros_client` WHERE 1

		$data=array('NOM_GROS_CLIENT'=>trim($nom),'PRENOM_GROS_CLIENT'=>trim($prenom),'TEL_GROS_CLIENT'=>trim($tel),'ADRESS_GROS_CLIENT'=>trim($localisation),'STATUT'=>trim($STATUT),'IS_CLIENT'=>trim($IS_CLIENT),'NIF'=>trim($NIF),'RS'=>trim($RaisonSociale),'RC'=>trim($RegistreCommerce),'ID_BRANCHE'=>trim($ID_BRANCHE));
		// print_r($data);die();
	...
	function liste()
	{
		...
		$order_by = isset($_POST['order']) ? ' ORDER BY '.$order_column[$_POST['order']['0']['column']] .'  '.$_POST['order']['0']['dir'] : 'ORDER  BY ADRESS_GROS_CLIENT  ASC';

		$search = !empty($_POST['search']['value']) ? (" AND  (RS LIKE '%$var_search%' OR PRENOM_GROS_CLIENT LIKE '%$var_search%',OR TEL_GROS_CLIENT LIKE '%$var_search%' OR NIF LIKE '%$var_search%' OR RC LIKE '%$var_search%' OR RS LIKE '%$var_search%' OR NOM_GROS_CLIENT LIKE '%$var_search%'OR STATUT LIKE '%$var_search%') ") : '';
