          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-body">
                ...
                <?php 
                if ($this->session->userdata('STR_DEPARTEMENT_CREATION')==1) {?>
                  <div class="row mb-2">
                  <div class="col-sm-5">
...
<script>
  ...
  $(document).ready(function() {
   ...
   table=$("#mytable").DataTable({
    ...
    "order":[],
    "ajax":{
      url:"<?php echo base_url('saisie/Departements/liste')?>",
      type:"POST"
    },
...
<script>
  ...
  function enr()
  {
   ...
   if (save_method=="add") 
   {
    url="<?php echo base_url('saisie/Departements/add')?>";
  }
  else
  {
    url="<?php echo base_url('saisie/Departements/update')?>";
  }
...
function edit_agence(id)
{
  ...
  $('#u_form')[0].reset();
  $.ajax({
    url : "<?php echo site_url('saisie/Departements/getOne')?>/" + id,
    type: "GET",
    dataType: "JSON",
    success: function(data)
    {
      ...
      $('[name="DESC_DEPARTEMENT"]').val(data.DESC_DEPARTEMENT);
      $('#u-modal').modal('show'); 
...
function supp_logic(id,is_actif)
{
  ...
  if (is_actif==1) {message="Voulez-vous désactiver?"} else {message="Voulez-vous activer?"}
   if (confirm(message)) {
    $.ajax({
      url : "<?php echo base_url('saisie/Departements/supp_logic')?>/"+id+'/'+is_actif,
      type: "POST",
      dataType: "JSON",
      success: function(data)
      {
...
function confirm_item(id,statut)
{
  ...
  message="Voulez-vous exécuté cette opération?";
   ...
   if (confirm(message)) {
    $.ajax({
      url : "<?php echo base_url('saisie/Departements/confirm_item')?>/"+id+'/'+statut,
      type: "POST",
      dataType: "JSON",
      success: function(data)
      {
                  ...
                  <div class="col-md-12">
                    <div class="form-group">
                      <div class="mb-3">
                        ...
                        <textarea class="form-control" name="DESC_DEPARTEMENT" id="DESC_DEPARTEMENT" rows="2" autofocus data-provide="typeahead"  autocomplete="off" autofocus></textarea>
