          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-body">
                ...
                <?php 
                if ($this->session->userdata('STR_AGENCE_CREATION')==1) {?>
                  <div class="row mb-2">
                    <div class="col-sm-5">
...
<script>
  ...
  $(document).ready(function() {
   ...
   table=$("#mytable").DataTable({
    ...
    "order":[],
    "ajax":{
      url:"<?php echo base_url('index.php/saisie/Agences/liste')?>",
      type:"POST"
    },
...
<script>
  ...
  function enr()
  {
   ...
   if (save_method=="add") 
   {
    url="<?php echo base_url('index.php/saisie/Agences/add')?>";
  }
  else
  {
    url="<?php echo base_url('index.php/saisie/Agences/update')?>";
  }
...
function confirm_item(id,statut)
{
  ...
  message="Voulez-vous exécuté cette opération?";
   ...
   if (confirm(message)) {
    $.ajax({
      url : "<?php echo base_url('index.php/saisie/Agences/confirm_item')?>/"+id+'/'+statut,
      type: "POST",
      dataType: "JSON",
      success: function(data)
      {
...
function edit_agence(id)
{
  ...
  $('#u_form')[0].reset();
  $.ajax({
    url : "<?php echo site_url('index.php/saisie/Agences/getOne')?>/" + id,
    type: "GET",
    dataType: "JSON",
    success: function(data)
    {
...
function supp_logic(id,is_actif)
{
  ...
  if (is_actif==1) {message="Voulez-vous désactiver?"} else {message="Voulez-vous activer?"}
   if (confirm(message)) {
    $.ajax({
      url : "<?php echo base_url('index.php/saisie/Agences/supp_logic')?>/"+id+'/'+is_actif,
      type: "POST",
      dataType: "JSON",
      success: function(data)
      {
