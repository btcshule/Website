...
<script>
  ...
  $(document).ready(function() {
   ...
   table=$("#mytable").DataTable({
    ...
    "order":[],
    "ajax":{
      url:"<?php echo base_url('saisie/Marques/liste')?>",
      type:"POST"
    },
...
<script>
  ...
  function enr()
  {
   ...
   if (save_method=="add") 
   {
    url="<?php echo base_url('saisie/Marques/add')?>";
  }
  else
  {
    url="<?php echo base_url('saisie/Marques/update')?>";
  }

  var formData = new FormData($('#marque_form')[0]);
...
function edit_marque(id)
{
  ...
  $('#marque_form')[0].reset();
  $.ajax({
    url : "<?php echo site_url('saisie/Marques/getOne')?>/" + id,
    type: "GET",
    dataType: "JSON",
    success: function(data)
    {

      $('[name="MARQUE_ID"]').val(data.MARQUE_ID);
      $('[name="DESC_MARQUE"]').val(data.DESC_MARQUE);
      $('#marque-modal').modal('show'); 
...
function supp_logic(id,is_actif)
{
  ...
  if (is_actif==1) {message="Voulez-vous désactiver?"} else {message="Voulez-vous activer?"}
   if (confirm(message)) {
    $.ajax({
      url : "<?php echo base_url('saisie/Marques/supp_logic')?>/"+id+'/'+is_actif,
      type: "POST",
      dataType: "JSON",
      success: function(data)
      {
...
<div id="marque-modal" class="modal fade" data-bs-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="info-header-modalLabel" aria-hidden="true">
  <div class="modal-dialog modal-sm">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="row">
          <div class="col-12">
            <form id="marque_form" method="POST">
              <div class="form-body">
                  ...
                  <div class="col-md-12">
                    <div class="form-group">
                      <div class="mb-3">
                        ...
                        <textarea class="form-control" name="DESC_MARQUE" id="DESC_MARQUE" rows="2" autofocus data-provide="typeahead"  autocomplete="off" autofocus></textarea>
