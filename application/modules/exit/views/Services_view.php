...
<script>
  ...
  $(document).ready(function() {
   ...
   table=$("#mytable").DataTable({
    ...
    "order":[],
    "ajax":{
      url:"<?php echo base_url('index.php/exit/Services/liste')?>",
      type:"POST"
    },
...
<script>
  function enr()
  {
    ...
    var MONTANT_SERVICE=$('#MONTANT_SERVICE').val();
    var statut = true;
    if (OCCUPATION_DESC=="") {
    ...
    if (MONTANT_SERVICE=="") {
      $("#errMONTANT_SERVICE").html("Le champ est obligatoire");
      statut=false;
    }else{
      $("#errMONTANT_SERVICE").html("");
    }
    if (statut==true) {
      ...
      if (SERVICE_ID=="") 
      {
        ...
        url="<?php echo base_url('index.php/exit/Services/add')?>";

      }
      else
      {
        url="<?php echo base_url('index.php/exit/Services/update')?>";
      }
      ...
      $.ajax({
        ...
        success: function(data)
        {
         if(data.status) 
         {
          ...
          alert_toast_validation('SUCCESS','Opération faite avec succès','success');
  ...
  function edit_emp(id)
{
 ...
 $('#SERVICE_ID').val(id);
    $('#emp-modal').modal('show');
  $.ajax({
    url : "<?php echo site_url('exit/Services/getOne')?>/" + id,
    type: "GET",
    dataType: "JSON",
    success: function(data)
    {
      $('[name="SERVICE_ID"]').val(data.PRODUCT_ID);
      $('[name="OCCUPATION_DESC"]').val(data.PRODUCT_DESC);
      $('[name="MONTANT_SERVICE"]').val(data.SERVICE_COST); 
  ...
  function supp_logic(id,is_actif)
  {
    ...
    if (is_actif==1) {message="Voulez-vous désactiver?"} else {message="Voulez-vous activer?"}
     if (confirm(message)) {
      $.ajax({
        url : "<?php echo base_url('index.php/exit/Services/del')?>/"+id+'/'+is_actif,
        type: "POST",
        dataType: "JSON",
        success: function(data)
        {
                ...
                <div class="row">
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      <div class="mb-3">
                        <label class="form-label">Description du service<span style="color: red;">*</span></label>
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      <div class="mb-3">
                        ...
                        <input type="text" autocomplete="off" name="MONTANT_SERVICE" class="form-control" id="MONTANT_SERVICE" autofocus>
                        <span class="text-danger" id="errMONTANT_SERVICE"></span>
                      </div>
                    </div>
                  </div>
                </div>
