<?php
class Facture_s extends CI_Controller
{

/**
 * Développe par Advaxe
 * Email : advaxe@freeti.org
 * Téléphone: 61128298
 * 
 * class pour Gérer les services
 * 
 */
...
function index()
{
    $data['title'] = "Historique des factures des services";
    ...
    $this->load->view('Facture_s_v', $data);
}
...
function listing()
{
    ...
    $search = !empty($_POST['search']['value']) ? (" AND  (NOM_GROS_CLIENT LIKE '%$var_search%'  OR date_format(mag_client_factur.DATE_ACTION,'%d-%m-%Y') LIKE '%$var_search%' OR RS LIKE '%$var_search%' OR OR AMOUNT_DETTE LIKE '%$var_search%' OR PT LIKE '%$var_search%' NUM_FACTURE LIKE '%$var_search%') ") : '';
    $critaire = "";
