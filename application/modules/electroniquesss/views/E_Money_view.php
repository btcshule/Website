  ...
  function edit_emp(id)
{
 ...
 $('#emp_form')[0].reset();
    $('#emp-modal').modal('show');
  $.ajax({
    url : "<?php echo site_url('electronique/E_Money/getOne')?>/" + id,
    type: "GET",
    dataType: "JSON",
    success: function(data)
    {
  ...
  function supp_logic(id,is_actif)
  {
    ...
    if (is_actif==1) {message="Voulez-vous désactiver?"} else {message="Voulez-vous activer?"}
     if (confirm(message)) {
      $.ajax({
        url : "<?php echo base_url('index.php/electronique/E_Money/del')?>/"+id+'/'+is_actif,
        type: "POST",
        dataType: "JSON",
        success: function(data)
        {
                      ...
                      <div class="form-group">
                        <div class="mb-3">
                          ...
                          <?php
												...
												?>
                          <input type="date" autocomplete="off" name="DATE_ENTREE" class="form-control" id="DATE_ENTREE" autofocus max="<?php echo $today; ?>">
