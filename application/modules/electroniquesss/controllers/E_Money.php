...
class E_Money extends CI_Controller
{
  ...
  function ajouter()
  {
    ...
    if (!empty($idmax)) {
      ...
      $SOLDE = $solde['SOLDE'];
      $SOLDE1 = $SOLDE+$ELECTRONIQUE;

    } else {
      $SOLDE1 =$ELECTRONIQUE+$CASH;
    }
    $data=array('LIBELLE'=>trim($COMMENTS),'MONTANT'=>trim($CASH),'COMMISSION'=>trim($ELECTRONIQUE),'SOLDE'=>trim($SOLDE1),'TYPE_AGENT'=>trim($TYPE_AGENT),'DATE_ENTREE'=>trim($DATE_ENTREE),'STATUT'=>trim($STATUT),'RESP_ELECT'=>trim($RESP_ELECT));
  ...
  function liste()
  {
    ...
    $query_principal=" SELECT `ID_E_BANK`, `LIBELLE`, `MONTANT`, `COMMISSION`, `SOLDE`, `TYPE_AGENT`, `DATE_ENTREE`, e_bank.STATUT, `DATE_ACTION`, NOM_EMP,PRENOM_EMP FROM `e_bank` JOIN employes ON employes.EMPLOYE_ID=e_bank.RESP_ELECT WHERE 1";
    $order_column=array('LIBELLE','MONTANT','COMMISSION','SOLDE','TYPE_AGENT','DATE_ENTREE','STATUT','DATE_ACTION','NOM_EMP');

    $order_by = isset($_POST['order']) ? ' ORDER BY '.$order_column[$_POST['order']['0']['column']] .'  '.$_POST['order']['0']['dir'] : 'ORDER  BY DATE_ACTION  DESC';

    $search = !empty($_POST['search']['value']) ? (" AND  (LIBELLE LIKE '%$var_search%' OR MONTANT LIKE '%var_search%' OR COMMISSION LIKE '%var_search%' OR SOLDE LIKE '%var_search%' OR TYPE_AGENT LIKE '%var_search%' OR e_bank.DATE_ENTREE LIKE '%var_search%' OR e_bank.STATUT LIKE '%var_search%' OR e_bank.DATE_ACTION LIKE '%var_search%' OR NOM_EMP LIKE '%var_search%') ") : '';
