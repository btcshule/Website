...
class Dashboard extends CI_Controller
{
  ...
  public function index(){
    ...
    public function get_rapport(){ 
      ...
      $x=2024; //2024
      $xx=$x-1; //2023
      $y=$this->Model->getRequeteOne('SELECT MAX(ID_LIVRE_CAISSE) AS MAX FROM data_livre_caisse WHERE YEAR(`DATE_ENTREE`)='.$xx);
      $yy=$this->Model->getRequeteOne('SELECT SOLDE FROM data_livre_caisse WHERE ID_LIVRE_CAISSE='.$y['MAX']);
      $t=$this->Model->getRequeteOne('SELECT MAX(ID_LIVRE_CAISSE) AS MAX FROM data_livre_caisse WHERE YEAR(`DATE_ENTREE`)='.$x);
      $tt=$this->Model->getRequeteOne('SELECT SOLDE FROM data_livre_caisse WHERE ID_LIVRE_CAISSE='.$t['MAX']);
      if (empty($t)){
        $montant=$yy;
      }else{
        $montant=$tt;
   ...
   $app_max = $this->Model->getRequeteOne("SELECT MAX(ID_LIVRE_APPRO) AS MAXIMUM FROM data_livre_approvisionnement");
    ...
    $clients=$this->Model->getRequeteOne("SELECT COUNT(GROS_CLIENT_ID) AS somme4 FROM `gros_client` WHERE 1");
    $frss=$this->Model->getRequeteOne("SELECT COUNT(ID_FOURNISSEUR) AS frss FROM `fournisseurs_sanya` WHERE 1");
    ...
    $total_serv=$services['somme3'];
       $nbre_fonctionnaire="
        ...
        {name:'Caisse Vente ".number_format($ventes['SOLDE'],0,',',' ')."', y:".$ventes['SOLDE'].",key:1},
...
$services.="{name:'Services : (".number_format($total_serv,0,',',' ')." FBU)', y:".$total_serv.",key:7,color:'#f77a0b'},";
    ...
    title: {
        text: '<b>Dashboard Général (Provisoire) </b><br>du ".date('d-m-Y')."',
        align: 'center'
    },
    subtitle: {
        text: '',
...
Highcharts.chart('container1', {
    ...
    title: {
       text: '<b>Rapport comparatif des caisses </b><br>Le ".date('d-m-Y')." ',
        align: 'center'
    },
    subtitle: {
        text:
...
Highcharts.chart('container2', {
    ...
    title: {
        text: '<b>Situation E-banking  </b><br>Le ".date('d-m-Y')." <br> Total= ".number_format($soldee['ELECTRONIQUE'] + $soldes['CASH'],0,'',' ')."',
        align: 'center'
    },
    subtitle: {
        text: '',
...
$rapp3="<script type=\"text/javascript\">
    ...
    title: {
        text: '<b>Rapport comparé Produits vs Services </b><br>Le ".date('d-m-Y')." <br> Total= ".number_format($total_vente,0,'',' ')."'
    },
