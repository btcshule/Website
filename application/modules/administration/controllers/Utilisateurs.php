...
class Utilisateurs extends CI_Controller
{
	...
	function index()
	{
		$data['page_title'] = "Gestion des Utilisateurs";
		...
		$data['profil1']=$this->Model->getRequete('SELECT `PROFILE_ID`, `DESC_PROFIL`, `IS_DELETED` FROM `profil` WHERE 1');
		$data['profil']=$this->Model->getRequete('SELECT `ID_USER_PROFIL`, `PROFIL_ID`, `DESC_USER_PROFIL`, `TACHES` FROM `user_profil` WHERE 1');
	...
	function liste()
	{
		...
		$search = !empty($_POST['search']['value']) ? (" AND  (PRENOM_EMP LIKE '%$var_search%' OR NOM_EMP LIKE '%$var_search%' OR DESC_PROFIL LIKE '%$var_search%' OR DESCRIPTION_BRANCH LIKE '%$var_search%' OR TEL_EMP LIKE '%$var_search%' OR EMAIL_EMP LIKE '%$var_search%' OR employes.STATUT LIKE '%$var_search%') ") : '';
