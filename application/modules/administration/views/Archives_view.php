...
<script>
  function new_record(){
    ...
    $('.modal-title').text('Importation des archives');
  }
</script>
...
<script>
  ...
  $(document).ready(function() {
   ...
   table=$("#mytable").DataTable({
    ...
    "order":[],
    "ajax":{
      url:"<?php echo base_url('index.php/administration/Archives/liste')?>",
      type:"POST"
    },
...
<script>
  function enr() {
    ...
    if (DOSSIER_ID == "") {
      $("#errDOSSIER_ID").html("Le champ est obligatoire");
      statut = false;
    } else {
      $("#errDOSSIER_ID").html("");
    }
    if (CONTENU == "") {
      $("#errCONTENU").html("Le champ est obligatoire");
      statut = false;
    } else {
      $("#errCONTENU").html("");
    }
    if (FICHIER == "") {
      $("#errFICHIER").html("Le champ est obligatoire");
      statut = false;
    } else {
      $("#errFICHIER").html("");
    }
    ...
    var url;
    var ID_ARCHIVES = $('#ID_ARCHIVES').val();

    if (ID_ARCHIVES == "") {
      url = "<?php echo base_url('index.php/administration/Archives/ajouter')?>";
    } else {
      url = "<?php echo base_url('index.php/administration/Archives/update')?>";
    }
  ...
  function edit_emp(id)
  {
    ...
    $.ajax({
      ...
      success: function(data)
      {
// SELECT `ID_ARCHIVES`, `NOM_EMP`, `PRENOM_EMP`, `EMAIL_EMP`, `TEL_EMP`, `PROFILE_ID`, `IS_USER_SYSTEM`, `MOT_DE_PASSE`, `USER_ID`, `DATE_CREATION`, `IS_ACTIF`, `IS_MUST_CHANGE_PWD`, `ID_BRANCHE` FROM `employes` WHERE 1
...
<script type="text/javascript">
  function afficherDocument(cheminFichier) {
    // var url = '<?= base_url() ?>' + cheminFichier;
    var url = '<?= base_url() ?>archives/uploads/' + cheminFichier;
    var windowObjectReference = window.open(
      url,
      "DescriptiveWindowName",
      "toolbar=yes,scrollbars=yes,resizable=yes,top=500,left=900,width=500,height=1200"
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="row">
          <div class="col-12">
            <form id="emp_form" method="POST">
              <div class="form-body">
                <input type="hidden" name="ID_ARCHIVES" id="ID_ARCHIVES">
                <div class="row">                  

                  <div class="col-md-4">
                    <div class="form-group">
                      <div class="mb-3">
                       <label>Dossier de l'archive</label>
                       <select name="DOSSIER_ID" id="DOSSIER_ID" class="form-control" 
                       onchange="getdesignations()">

                       <option value="">---Séléctionner---</option>
                       <?php
                       foreach($dossiers as $pro)
                       {
                        if ($pro['DOSSIER_ID']==set_value('DOSSIER_ID'))
                        {
                          ?>
                          <option value="<?=$pro['DOSSIER_ID'] ?>" selected><?=$pro['DOSSIER_ARCHIVE']; ?></option>
                          <?php
                        }
                        else
                        {
                          ?>
                          <option value="<?= $pro['DOSSIER_ID'] ?>"><?=$pro['DOSSIER_ARCHIVE']; ?></option>
                          <?php
                        }
                      }
                      ?>
              ...
              <div class="col-md-4">
                <div class="form-group">
                  <div class="mb-3">
                    <label class="form-label">Contenu de l'archive <span style="color: red;">*</span></label>
                    <input type="text" autocomplete="off" name="CONTENU" class="form-control" id="CONTENU" autofocus>
                    <span class="text-danger" id="errCONTENU"></span>
                  </div>
                </div>
              </div>
                ...
                <div class="form-group">
                  <div class="mb-3">
                    <label class="form-label">Fichier d'archive <span style="color: red;">*</span></label>
                    <input type="file" autocomplete="off" name="FICHIER" class="form-control" id="FICHIER" autofocus>
                    <span class="text-danger" id="errFICHIER"></span>
                  </div>
                </div>
              </div>
        ...
        <div class="modal-footer">
          ...
          <button type="button" id="btnSaved" onclick="enr()" class="btn btn-info">Enregistrer</button>
          <!-- <button type="button" id="btnSaved" class="btn btn-info">Enregistrer</button> -->
        </div>
      </div>
    </div>
  </form>
