...
function update()
{
    ...
    if ($STATUT_ID==1) {
        $this->Model->update('mag_client_factur',array('GROS_CLIENT_FACT_ID'=>$ID_CLIENT_FAC),array('STATUT'=>$STATUT_ID));

    }else{
        $array = $this->Model->getRequete("SELECT * FROM mag_ventes_services where  GROS_CLIENT_FACT_ID=" . $ID_CLIENT_FAC);
