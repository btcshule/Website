...
class Magasin_sales extends CI_Controller
{
  ...
  function index()
  {
    $data['page_title']="Gestionnaire des ventes des marchandises";
    ...
    <li class="breadcrumb-item active" aria-current="page"> Vente des marchandises</li>
    </ol>
    </nav>';
 ...
 function get_data($id = "",$typ_prod = "")
  {
    $STATUT=1;
    $data_stock = $this->Model->getOne('stock_fiche', array('PRODUCT_ID' => $id,'TYPE_STOCK'=>$typ_prod,'STATUT'=>$STATUT));
    echo json_encode($data_stock);
  }
  function get_data_secre($id = "",$typ_prod = "")
  {
  ...
  public function add_in_cart()
  {
    ...
    if ($UNITE_MESURE==0) {
      $secr=$this->Model->getRequeteOne('SELECT * FROM products WHERE products.PRODUCT_ID='.$DESIGNATION );
    ...
    foreach ($this->cart->contents() as $item) :
      if (preg_match('/FILE/', $item['typecartitem'])) {
        if ($item['UNITE_MESURE']==0) {
          $DES = $this->Model->getRequeteOne('SELECT * FROM products  WHERE PRODUCT_ID=' . $item['SECR_STOCK_ID']);
...
{ 
  ...
  if ($value['UNITE_MESURE']!=0){
    $reste=$this->Model->getRequeteOne('SELECT * FROM stock_secretariat WHERE PRODUCT_ID='.$value['DESIGNATION'].' AND TYPE_STOCK='.$value['type_produit']);
    $NEW_QNTE=$reste['QNTE']-$value['QUANTITEUN'];
    $arrayds=array(
     'QNTE'=>$NEW_QNTE
   );
...
// //mettre a jour la fiche de stock
 //  $qte_stock = $this->Model->getRequete('SELECT `SECR_STOCK_ID`, `PRODUCT_ID`, `TYPE_STOCK`, `QNTE`, `PU`, `STATUT`, `DATE_INSERTION` FROM `stock_fiche` WHERE  PRODUCT_ID="'.$value['DESIGNATION'].'" AND TYPE_STOCK="'.$value['type_produit'].'" ORDER BY DATE_INSERTION ASC');
 //  foreach ($qte_stock as $key) {
 //  $Qte_restante=$key['QNTE']-$value['QUANTITEUN'];
 //  if ($Qte_restante != 0) 
 //  {
 //      $array1=array(
 //     'QNTE'=>$Qte_restante
 //   );
 //  $update=$this->Model->update('stock_fiche',array('PRODUCT_ID'=>$value['DESIGNATION'],'TYPE_STOCK'=>$value['type_produit']),$array1);
 //  }else
 //  {
