<?php
class Sales_into_manager extends CI_Controller
{
...
function index()
{
    $data['title'] = "Sales and commands";
    $date1=date('Y-m-d');
    $debts=$this->Model->getRequeteOne('SELECT SUM(AMOUNT_DETTE)AS dette ,DATE_ACTION,STATUT FROM mag_client_facture WHERE STATUT=1 AND date_format(DATE_ACTION,"%Y-%m-%d")="'.$date1.'"');
    $data['debts']=$debts['dette'];
    $date2=date('Y-m-d');
    $paid=$this->Model->getRequeteOne('SELECT SUM(NET_PAID)AS PAID ,DATE_ACTION,STATUT FROM mag_client_facture JOIN mag_ventes_produits ON mag_ventes_produits.GROS_CLIENT_FACT_ID=mag_client_facture.GROS_CLIENT_FACT_ID  WHERE STATUT=1 AND  date_format(DATE_ACTION,"%Y-%m-%d") LIKE "'.$date2.'"');
    $data['paid']=$paid['PAID'];

    $date2=date('Y-m-d');
    $detpayed=$this->Model->getRequeteOne('SELECT SUM(`AMOUNT_PAID_DETTE`)as dettePAID FROM `dettes_payees` WHERE DATE_FORMAT(`DATE_OPERATION`,"%Y-%m-%d") LIKE "'.$date2.'"');
    $data['detpayed']=$detpayed['dettePAID'];

    $this->load->view('Sales_into_manager_view', $data);
}
...
function listing()
{
    ...
    foreach ($fetch_data as $row) {
        ...
        $sub_array[] =  number_format($row->AMOUNT_DETTE, 2, ',', ' ');
        $sub_array[] = '<center>
