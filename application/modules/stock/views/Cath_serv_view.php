          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-body">
                <div class="row mb-2">
                  <div class="col-sm-5">
                    <a href="javascript:void(0);" onclick="new_record()" class="btn btn-primary mb-2"><i class="mdi mdi-plus-circle me-2"></i> Nouvelle catégorie</a>
                  </div>
                </div> 
...
<script>
  function new_record(){
    ...
    $('.modal-title').text('Ajouter une nouvelle catégorie');
  }
</script>
...
<script>
  ...
  $(document).ready(function() {
   ...
   table=$("#mytable").DataTable({
    ...
    "order":[],
    "ajax":{
      url:"<?php echo base_url('index.php/stock/Cath_service/liste')?>",
      type:"POST"
    },
...
<script>
  function enr()
  {
    var UNITE_DESCR=$('#UNITE_DESCR').val();

    var statut = true;
    if (UNITE_DESCR=="") {
      $("#errUNITE_DESCR").html("Le champ est obligatoire");
      statut=false;
    }else{
      $("#errUNITE_DESCR").html("");
    }
    ...
    if (statut==true) {
      ...
      if (CATH_ID=="") 
      {
        ...
        url="<?php echo base_url('index.php/stock/Cath_service/ajouter')?>";
      }
      else
      {
        url="<?php echo base_url('index.php/stock/Cath_service/update')?>";
      }
  ...
  function edit_emp(id)
{
 ...
 $('#emp_form')[0].reset();
    $('#emp-modal').modal('show');
  $.ajax({
    url : "<?php echo site_url('index.php/stock/Cath_service/getOne')?>/" + id,
    type: "GET",
    dataType: "JSON",
    success: function(data)
    {
      ...
      $('[name="UNITE_DESCR"]').val(data.UNITE_DESCR); 
      $('[name="GROS_UNIT_ID"]').val(data.GROS_UNIT_ID); 
  ...
  function supp_logic(id,is_actif)
  {
    ...
    if (is_actif==1) {message="Voulez-vous désactiver?"} else {message="Voulez-vous activer?"}
     if (confirm(message)) {
      $.ajax({
        url : "<?php echo base_url('index.php/stock/Cath_service/del')?>/"+id+'/'+is_actif,
        type: "POST",
        dataType: "JSON",
        success: function(data)
        {
                  ...
                  <div class="col-md-12">
                    <div class="form-group">
                      <div class="mb-3">
                        ...
                        <input type="text" autocomplete="off" name="UNITE_DESCR" class="form-control" id="UNITE_DESCR" autofocus>
                        <span class="text-danger" id="errUNITE_DESCR"></span>
                      </div>
                    </div>
                  </div>
