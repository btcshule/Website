...
function ajouter()
  {    
    ...
    $data_stock=$this->Model->getOne('stock_secretariat',array('PRODUCT_ID'=>$GROS_PRODUIT_ID,'TYPE_STOCK'=>$TYPE_STOCK));
    $QUANT_DISPO=$data_stock['QNTE'];
    ...
    $data_inserer=array(
      ...
      'DATE_INSERTION'=>$hourtime
        ...
        $data_fichestock=$this->Model->getOne('stock_fiche',array('PRODUCT_ID'=>$GROS_PRODUIT_ID,'TYPE_STOCK'=>$TYPE_STOCK,'PU'=>$PRIX_ACHAT));
        if (!empty($data_fichestock)) 
        {
          $qte_exist=$data_fichestock['QNTE'];
          $New_qte=$qte_exist+$QUANTITE;
          $data_upfiche=array(
            'QNTE'=>$New_qte,
          );
        }
    ...
    $invaleur=$this->Model->create('stock_valeur',$data_invaleur);
    $update = $this->Model->update('stock_secretariat', array('PRODUCT_ID' => $GROS_PRODUIT_ID, 'TYPE_STOCK' => $TYPE_STOCK), $data_update); 
    if (!empty($data_fichestock)) {
      $upfiche = $this->Model->update('stock_fiche', array('PRODUCT_ID' => $GROS_PRODUIT_ID, 'TYPE_STOCK' => $TYPE_STOCK, 'PU' => $PRIX_ACHAT), $data_upfiche); 
    }else{
      $infiche=$this->Model->create('stock_fiche',$data_infiche);
    }
    if ($creation && $update)
    {
  ...
  function liste()
  {
    ...
    WHERE
    DATE(gros_entrees_stock.DATE_INSERTION) =".$todayDate;
