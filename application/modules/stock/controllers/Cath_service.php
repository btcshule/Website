...
class Cath_service extends CI_Controller
{
	...
	{    
		...
		$this->Model->create('cathegories_services',$data);
	...
	function liste()
	{
		...
		$query_principal="SELECT `CATH_ID`, `CATH_DESC`,STATUT FROM `cathegories_services` WHERE 1";
	...
	{
		$value = ($stat==1) ? 0 : 1 ;
		$this->Model->update('cathegories_services',array('CATH_ID'=>$id),array('STATUT'=>$value));
		echo json_encode(array('status'=>true));
	}
	function getOne($id)
	{
		$data=$this->Model->getRequeteOne('SELECT `CATH_ID`, `UNITE_DESCR` `STATUT` FROM `cathegories_services` WHERE CATH_ID='.$id);
		echo json_encode($data);
	}
	function update()
	{
		$UNITE_DESCR=$this->input->post('UNITE_DESCR');
		$this->Model->update('cathegories_services',array('CATH_ID'=>$this->input->post('GROS_UNIT_ID')),array('UNITE_DESCR'=>$UNITE_DESCR));
		echo json_encode(array('status'=>true));
	}
}
?>