...
class E_money extends CI_Controller
{
  ...
  function index()
  {
    ...
    <li class="breadcrumb-item"><a href="#">E-bank Money</a></li>
    <li class="breadcrumb-item active" aria-current="page">Encaissements</li>
    </ol>
    </nav>';
  ...
  function ajouter()
  {
    ...
    $FRAIS_TRANS=$this->input->post('FRAIS_TRANS');
    ...
    // // print_r($idmax);die();
    // if (!empty($idmax)) {
    //   $solde = $this->Model->getRequeteOne("SELECT MONTANT,SOLDE,ELECTRONIQUE FROM e_bank WHERE ID_E_BANK=".$idmax);
    //   $SOLDE = $solde['SOLDE'];
    //   $ELE = $solde['ELECTRONIQUE'];
    //   $CAS= $solde['MONTANT'];
      
    //   $ELE_REST=$ELE-$ELECTRONIQUE;
    //   $ELE_ENR=$ELE_REST+$COMMISSION;
    //   $CASH_ENR=$CAS+$CASH;
    //   $SOLDE1 = $ELE_ENR+$CASH_ENR;
    // } else {
    //   $SOLDE1 =$COMMISSION+$CASH;
    // }
    
        $max_s = $this->Model->getRequeteOne("SELECT MAX(ID_SUIVI_EBANK) AS MAXIMUM FROM suivi_ebank");
        $idmax = $max_s['MAXIMUM'];
      
      if (!empty($idmax)) {
       $solde = $this->Model->getRequeteOne("SELECT CASH,ELECTRONIQUE FROM suivi_ebank WHERE ID_SUIVI_EBANK=".$idmax);
     
      $ELE = $solde['ELECTRONIQUE'];
      $CAS= $solde['CASH'];
      $ELE_REST=$ELE-$ELECTRONIQUE;
      $ELE_ENR=$ELE_REST+$COMMISSION;
      $CASH_ENR=$CAS+$CASH+$FRAIS_TRANS;
    } else {
      $ELE_ENR=$ELECTRONIQUE+$COMMISSION;
      $CASH_ENR=$CASH+$FRAIS_TRANS;
    }
    $data=array('LIBELLE'=>trim($COMMENTS),'MONTANT'=>trim($CASH),'ELECTRONIQUE'=>trim($ELECTRONIQUE),'FRAIS_TRANS'=>trim($FRAIS_TRANS),'COMMISSION'=>trim($COMMISSION),'TYPE_AGENT'=>trim($TYPE_AGENT),'DATE_ENTREE'=>trim($DATE_ENTREE),'TELEPHONE'=>trim($TELEPHONE),'STATUT'=>trim($STATUT),'RESP_ELECT'=>trim($RESP_ELECT));
    $data_suivi= array('CASH' =>trim($CASH_ENR),'ELECTRONIQUE'=>trim($ELE_ENR),'RESP_ELECT'=>trim($RESP_ELECT));
  ...
  function liste()
  {
    ...
    $query_principal=" SELECT `ID_E_BANK`, `LIBELLE`, `MONTANT`,ELECTRONIQUE,FRAIS_TRANS, `COMMISSION`, `TYPE_AGENT`, `DATE_ENTREE`, e_bank.STATUT, `DATE_ACTION`, NOM_EMP,PRENOM_EMP,TELEPHONE FROM `e_bank` JOIN employes ON employes.EMPLOYE_ID=e_bank.RESP_ELECT WHERE 1";
    $order_column=array('LIBELLE','FRAIS_TRANS','MONTANT','COMMISSION','ELECTRONIQUE','SOLDE','TELEPHONE','TYPE_AGENT','DATE_ENTREE','STATUT','DATE_ACTION','NOM_EMP');

    $order_by = isset($_POST['order']) ? ' ORDER BY '.$order_column[$_POST['order']['0']['column']] .'  '.$_POST['order']['0']['dir'] : 'ORDER  BY DATE_ACTION  DESC';

   $search = !empty($_POST['search']['value']) ? (" AND  (LIBELLE LIKE '%$var_search%' OR MONTANT LIKE '%$var_search%' OR COMMISSION LIKE '%$var_search%' OR ELECTRONIQUE LIKE '%$var_search%' OR FRAIS_TRANS LIKE '%$var_search%' OR TELEPHONE LIKE '%$var_search%' OR TYPE_AGENT LIKE '%$var_search%' OR DATE_ENTREE LIKE '%$var_search%' OR e_bank.STATUT LIKE '%$var_search%' OR DATE_ACTION LIKE '%$var_search%' OR PRENOM_EMP LIKE '%$var_search%' OR NOM_EMP LIKE '%$var_search%') ") : '';
