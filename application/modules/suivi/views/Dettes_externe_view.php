...
<script>
  ...
  $(document).ready(function() {
   ...
   table=$("#mytable").DataTable({
    ...
    "order":[],
    "ajax":{
      url:"<?php echo base_url('index.php/suivi/Dettes_externes/liste')?>",
      type:"POST"
    },
...
<script>
  function enr()
  {
    ...
    if (statut==true) {
      ...
      var url;
      var ID_DETTE_EXTERNE=$('#ID_DETTE_EXTERNE').val();
      
      if (ID_DETTE_EXTERNE=="") 
      {

        url="<?php echo base_url('index.php/suivi/Dettes_externes/ajouter')?>";
      }
      else
      {
        url="<?php echo base_url('index.php/suivi/Dettes_externes/ajouter')?>";
  ...
  function edit_emp(id)
  {
    save_method = 'update';
    $('#emp_form22')[0].reset();
    $('#emp-modal22').modal('show');
    $.ajax({
      url : "<?php echo site_url('index.php/suivi/Dettes_externes/getOne')?>/" + id,
      type: "GET",
      dataType: "JSON",
      success: function(data)
      {
        ...
        $('[name="ID_DETTE_EXTERNE"]').val(data.ID_DETTE_EXTERNE); 
      ...
      { 
        ...
        $('.modal-title').text('Enregistrer la récolte après séchage'); 
      }
  function supp_logic(id)
  {
    ...
    if (is_actif==1) {message="Cette action n'est pas autorisée"} else {message="Voulez-vous liquider cette dette?"}
     if (confirm(message)) {
      $.ajax({
        url : "<?php echo base_url('index.php/suivi/Dettes_externes/del')?>/"+id,
        type: "POST",
        dataType: "JSON",
        success: function(data)
        {
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      <div class="mb-3">
                        ...
                        ?>
                        <input type="date" autocomplete="off" name="DATE_DETTE" class="form-control" id="DATE_DETTE" autofocus max="<?php echo $today; ?>">
