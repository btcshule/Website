...
class Entrees extends CI_Controller
{
  ...
  function index()
  {
    ...
    $data['page_title']="Montant issu des ventes";
 ...
 $data['imputation']=$this->Model->getRequete('SELECT `ID_IMPUTATION`, `CODE_IMPUTATION`, `DESC_IMPUTATION` FROM `imputations` WHERE 1 ORDER BY CODE_IMPUTATION ASC');
    ...
    $this->load->view('Entrees_list_view',$data);
  }
...
if (empty($idmax)) {
    ...
    $solde = $this->Model->getRequeteOne("SELECT SOLDE FROM data_livre_caisse WHERE STATUT=1 AND ID_LIVRE_CAISSE=".$idmax);
    $SOLDE = $solde['SOLDE'];
}
...
$data = array(
    'DATE_ENTREE' => trim($DATE_ENTREE),
    'LIBELLE' => trim($LIBELLE),
    'IMPUTATION' => trim($IMPUTATION),
    'DEBIT' => trim($MONTANT),
    'CREDIT' => trim($CREDIT),
    'SOLDE' => trim($SOLDE1),
    'RESP_CAISSE' => trim($RESP_CAISSE),
    'EST_ENTREE' => trim($EST_ENTREE)
);
...
echo json_encode(array('status' => true));
  ...
  function liste()
  {
    ...
    $query_principal="SELECT `ID_LIVRE_CAISSE`, `LIBELLE`, NOM_EMP, PRENOM_EMP, `DEBIT`,CODE_IMPUTATION, `CREDIT`, SOLDE, `DATE_ENTREE`, `DATE_ACTION`,data_livre_caisse.STATUT FROM `data_livre_caisse` JOIN employes ON employes.EMPLOYE_ID=data_livre_caisse.RESP_CAISSE  JOIN imputations ON imputations.ID_IMPUTATION=data_livre_caisse.IMPUTATION WHERE EST_ENTREE=1";
    $order_column=array('LIBELLE','IMPUTATION','DEBIT','CREDIT','SOLDE','DATE_ACTION','NOM_EMP','PRENOM_EMP');
    ...
    foreach ($fetch_data as $row) 
    {
      ...
      $date_formatee = date('d-m-y', strtotime($date_entree));
      $sub_array[] = $date_formatee;
