...
class Sorties extends CI_Controller
{
 ...
 function del($id, $stat)
  ...
  function liste()
  {
    ...
    $query_principal="SELECT `ID_LIVRE_CAISSE`, `LIBELLE`, `CODE_IMPUTATION`, NOM_EMP, PRENOM_EMP, `DEBIT`, `CREDIT`, SOLDE, `DATE_ENTREE`, `DATE_ACTION`,data_livre_caisse.STATUT FROM `data_livre_caisse` JOIN employes ON employes.EMPLOYE_ID=data_livre_caisse.RESP_CAISSE JOIN imputations ON imputations.ID_IMPUTATION=data_livre_caisse.IMPUTATION WHERE EST_ENTREE=0";
    ...
    $search = !empty($_POST['search']['value']) ? (" AND  (LIBELLE LIKE '%$var_search%' OR CREDIT LIKE '%$var_search%' OR CODE_IMPUTATION LIKE '%$var_search%' OR DATE_ENTREE LIKE '%$var_search%' OR PRENOM_EMP LIKE '%$var_search%') ") : '';
