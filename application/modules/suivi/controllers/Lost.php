...
class Lost extends CI_Controller
{
...

  {
    ...
    $this->make_bread->add('Reporting', "suivi/Materiels", 0);
...
function ajouter()
{
      ...
      else
    {
     ...
     redirect(base_url('index.php/suivi/Lost/listing')); 
}

function listing(){

  $entrees= $this->Model->getRequete('SELECT  `DESC_MATERIEL`, `QTE_MATERIEL`,DATE_MATERIEL, users.USER_PRENOM FROM `losts` JOIN users ON users.USER_ID=losts.RESP_MATERIEL  WHERE 1 ORDER BY DATE_MATERIEL DESC');
     $i=0;
    ...
    $this->table->set_heading('#','Date','Materiel','Quantity');
    $this->table->set_template($template);
    $data['ourclient']=$datas;
    $data['breadcrumb'] = $this->make_bread->output();

  $this->load->view('Lost_list_view',$data);
 }
