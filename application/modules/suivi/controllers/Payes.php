...
class Payes extends CI_Controller
{
	...
	function index()
	{
		$data['page_title']="Gestionnaire des créances payées";
		...
		<li class="breadcrumb-item active" aria-current="page">Créances payées</li>
		</ol>
		</nav>';
...
// 		 $data['client']=$this->Model->getRequete('SELECT `ID_CLIENT`, `NOM_CLIENT`, `PRENOM_CLIENT`, `TEL_CLIENT`, `LOC_CLIENT`, `STATUT`, `DATE_INSERTION` FROM `clients_sanya` WHERE 1 ORDER BY NOM_CLIENT ASC');		
		$this->load->view('Payes_view',$data);
	}
