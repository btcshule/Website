...
class Services extends CI_Controller
{
  ...
  function index()
  {
    ...
    $data['biens']=$this->Model->getRequete('SELECT products.PRODUCT_ID, products.PRODUCT_DESC FROM `stock_secretariat` JOIN products On products.PRODUCT_ID=stock_secretariat.PRODUCT_ID WHERE TYPE_STOCK=1 AND QNTE!=0');    
  ...
  {    
    ...
    // $qtee = $this->Model->getRequeteOne("SELECT SECR_STOCK_ID, QNTE FROM `stock_secretariat` WHERE TYPE_STOCK=1 and PRODUCT_ID=" . $PRODUCT_ID); 
    // $qte_r=$qtee['QNTE']-1;
    // $id=$qtee['SECR_STOCK_ID'];
    // print_r($id);die();
    $data_inserer=array(
      'DESIGNATION'=>$LIBELLE,
      'ID_USER'=>$ID_USER,
    ...
    // $update = $this->Model->update('stock_secretariat', ['SECR_STOCK_ID' => $id], $data_update);   
     if ($creation)
    {
  ...
  function liste()
  {
    ...
    // SELECT `ID_SERVICE`, `DESIGNATION`, `ID_USER`, `BRANCHE`, `ID_CLIENT`, `PRODUCT_ID`, `PC`, `MO`, `AF`, `NAF`, `PT`, `COMMENTAIRE`, `VA`, `PV`, `DATE_CREATION`, `STATUT` FROM `services` WHERE 1

    $query_principal="SELECT `ID_SERVICE`, `DESIGNATION`, PRENOM_EMP,NOM_EMP, `DESCRIPTION_BRANCH`, `RS`, `PC`, `MO`, `AF`,NAF, `PT`, `VA`, `PV`,COMMENTAIRE, PRODUCT_ID, services.DATE_CREATION, services.STATUT FROM `services` JOIN employes ON employes.EMPLOYE_ID=services.ID_USER JOIN gros_client ON gros_client.GROS_CLIENT_ID=services.ID_CLIENT JOIN sanya_branches ON sanya_branches.ID_BRANCHE=services.BRANCHE WHERE 1";
    ...
    $search = !empty($_POST['search']['value']) ? (" AND  (DESIGNATION LIKE '%$var_search%' OR PRENOM_EMP LIKE '%$var_search%' OR DESCRIPTION_BRANCH LIKE '%$var_search%' OR RS LIKE '%$var_search%' OR PC LIKE '%$var_search%' OR MO LIKE '%$var_search%' OR AF LIKE '%$var_search%' OR PT LIKE '%$var_search%' OR VA LIKE '%$var_search%' OR PV LIKE '%$var_search%' OR COMMENTAIRE LIKE '%$var_search%' OR services.STATUT LIKE '%$var_search%' OR services.DATE_CREATION LIKE '%$var_search%' OR products.PRODUCT_DESC LIKE '%$var_search%' OR NAF LIKE '%$var_search%' OR NOM_EMP LIKE '%$var_search%') ") : '';
