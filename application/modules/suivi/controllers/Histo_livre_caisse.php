...
class Histo_livre_caisse extends CI_Controller
{
  ...
  function index()
  {
    $data['page_title']="Livre de caisse";
    ...
    <li class="breadcrumb-item active" aria-current="page">Livre Caisse</li>
    </ol>
    </nav>';
    $this->load->view('Histo_livre_caisse_view',$data);
  }
  ...
  function liste()
  {
    ...
    $query_principal="SELECT `ID_LIVRE_CAISSE`, `LIBELLE`, `CODE_IMPUTATION`, `DEBIT`, `CREDIT`, `SOLDE`, `DATE_ENTREE`, `EST_ENTREE`, data_livre_caisse.STATUT, `DATE_ACTION`, employes.NOM_EMP, employes.PRENOM_EMP FROM `data_livre_caisse` LEFT JOIN employes ON employes.EMPLOYE_ID=data_livre_caisse.RESP_CAISSE JOIN imputations ON imputations.ID_IMPUTATION=data_livre_caisse.IMPUTATION WHERE 1";
    $order_column=array('LIBELLE','CODE_IMPUTATION','DEBIT','CREDIT','DATE_ACTION','NOM_EMP','PRENOM_EMP');

    $order_by = isset($_POST['order']) ? ' ORDER BY '.$order_column[$_POST['order']['0']['column']] .'  '.$_POST['order']['0']['dir'] : 'ORDER  BY ID_LIVRE_CAISSE  DESC';
