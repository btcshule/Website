          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-body">
                ...
                <?php 
                if ($this->session->userdata('UNITE_CREATION')==1) {?>
...
<script>
  ...
  $(document).ready(function() {
   ...
   table=$("#mytable").DataTable({
    ...
    "order":[],
    "ajax":{
      url:"<?php echo base_url('saisie/Unites/liste')?>",
      type:"POST"
    },
...
<script>
  ...
  function enr()
  {
   ...
   if (save_method=="add") 
   {
    url="<?php echo base_url('saisie/Unites/add')?>";
  }
  else
  {
    url="<?php echo base_url('saisie/Unites/update')?>";
  }
...
function edit_unite(id)
{
  ...
  $('#u_form')[0].reset();
  $.ajax({
    url : "<?php echo site_url('saisie/Unites/getOne')?>/" + id,
    type: "GET",
    dataType: "JSON",
    success: function(data)
    {
      ...
      $('[name="UNITE_DESC"]').val(data.UNITE_DESC);
      $('#u-modal').modal('show'); 
...
function supp_logic(id,is_actif)
{
  ...
  message="Voulez-vous exécuté cette opération?";
   ...
   if (confirm(message)) {
    $.ajax({
      url : "<?php echo base_url('saisie/Unites/supp_logic')?>/"+id+'/'+is_actif,
      type: "POST",
      dataType: "JSON",
      success: function(data)
      {
...
function confirm_item(id,statut)
{
  ...
  message="Voulez-vous exécuté cette opération?";
   ...
   if (confirm(message)) {
    $.ajax({
      url : "<?php echo base_url('saisie/Unites/confirm_item')?>/"+id+'/'+statut,
      type: "POST",
      dataType: "JSON",
      success: function(data)
      {
                  ...
                  <div class="col-md-12">
                    <div class="form-group">
                      <div class="mb-3">
                        ...
                        <textarea class="form-control" name="UNITE_DESC" id="UNITE_DESC" rows="2" autofocus data-provide="typeahead"  autocomplete="off" autofocus></textarea>
