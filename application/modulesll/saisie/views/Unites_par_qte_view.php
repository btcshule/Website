          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-body">
                ...
                <?php 
                if ($this->session->userdata('UNITE_PAR_QTE_CREATION')==1) {?>
...
<script>
  ...
  $(document).ready(function() {
   ...
   table=$("#mytable").DataTable({
    ...
    "order":[],
    "ajax":{
      url:"<?php echo base_url('saisie/Unites_par_qte/liste')?>",
      type:"POST"
    },
...
<script>
  ...
  function enr()
  {
   ...
   if (save_method=="add") 
   {
    url="<?php echo base_url('saisie/Unites_par_qte/add')?>";
  }
  else
  {
    url="<?php echo base_url('saisie/Unites_par_qte/update')?>";
  }
...
function edit_unite(id)
{
  ...
  $('#u_form')[0].reset();
  $.ajax({
    url : "<?php echo site_url('saisie/Unites_par_qte/getOne')?>/" + id,
    type: "GET",
    dataType: "JSON",
    success: function(data)
    {
      ...
      $('[name="UNITE_QTE_DESC"]').val(data.UNITE_QTE_DESC);
      $('#u-modal').modal('show'); 
...
function supp_logic(id,is_actif)
{
  ...
  if (is_actif==1) {message="Voulez-vous désactiver?"} else {message="Voulez-vous activer?"}
   if (confirm(message)) {
    $.ajax({
      url : "<?php echo base_url('saisie/Unites_par_qte/supp_logic')?>/"+id+'/'+is_actif,
      type: "POST",
      dataType: "JSON",
      success: function(data)
      {
        ...
        <div class="row">
          <div class="col-12">
            <form id="u_form" method="POST">
              <div class="form-body">
                <input type="hidden" name="UNITE_QTE_ID" id="UNITE_QTE_ID">
                <div class="row">
                  <div class="col-md-12">
                    <div class="form-group">
                      <div class="mb-3">
                        <label class="form-label"> Description<span style="color:red;">*</span></label>
                        <textarea class="form-control" name="UNITE_QTE_DESC" id="UNITE_QTE_DESC" rows="2" autofocus data-provide="typeahead"  autocomplete="off" autofocus></textarea>
