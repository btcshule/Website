...
<script>
  ...
  $(document).ready(function() {
   ...
   table=$("#mytable").DataTable({
    ...
    "order":[],
    "ajax":{
      url:"<?php echo base_url('saisie/Categories/liste')?>",
      type:"POST"
    },
...
<script>
  ...
  function enr()
  {
   ...
   if (save_method=="add") 
   {
    url="<?php echo base_url('saisie/Categories/add')?>";
  }
  else
  {
    url="<?php echo base_url('saisie/Categories/update')?>";
  }

  var formData = new FormData($('#fam_form')[0]);
...
function edit_fam(id)
{
  ...
  $('#fam_form')[0].reset();
  $.ajax({
    url : "<?php echo site_url('saisie/Categories/getOne')?>/" + id,
    type: "GET",
    dataType: "JSON",
    success: function(data)
    {

      $('[name="FAMILLE_ID"]').val(data.FAMILLE_ID);
      $('[name="DESC_FAMILLE"]').val(data.DESC_FAMILLE);
      $('[name="GROUPE_ID"]').val(data.GROUPE_ID);
      $('#fam-modal').modal('show'); 
...
function supp_logic(id,is_actif)
{
  ...
  if (is_actif==1) {message="Voulez-vous désactiver?"} else {message="Voulez-vous activer?"}
   if (confirm(message)) {
    $.ajax({
      url : "<?php echo base_url('saisie/Famille/supp_logic')?>/"+id+'/'+is_actif,
      type: "POST",
      dataType: "JSON",
      success: function(data)
      {
...
<div id="fam-modal" class="modal fade" data-bs-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="info-header-modalLabel" aria-hidden="true">
                ...
                <div class="row">
                  ...
                  <div class="col-md-12">
                    <div class="form-group">
                      <div class="mb-3">
                        <label class="form-label"> Groupe<span style="color:red;">*</span></label>
                        <select class="form-control" id="GROUPE_ID" name="GROUPE_ID">
                          <option value="0">Choisir...</option>
                          <?php 
                          foreach ($groupes as $key) {
                            # code...
                            echo "<option value='".$key['GROUPE_ID']."'>".$key['GROUPE_DESC']."</option>";
                          }
                          ?>
                        </select>
                  ...
                  <div class="col-md-12">
                    <div class="form-group">
                      <div class="mb-3">
                        ...
                        <textarea class="form-control" name="DESC_FAMILLE" id="DESC_FAMILLE" rows="2" autofocus data-provide="typeahead"  autocomplete="off" autofocus></textarea>
                        <span class="help-block"></span>
                      </div>
                    </div>
                  </div>
