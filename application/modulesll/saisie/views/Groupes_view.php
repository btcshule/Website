...
<script>
  ...
  $(document).ready(function() {
   ...
   table=$("#mytable").DataTable({
    ...
    "order":[],
    "ajax":{
      url:"<?php echo base_url('saisie/Groupes/liste')?>",
      type:"POST"
    },
...
<script>
  ...
  function enr()
  {
   ...
   if (save_method=="add") 
   {
    url="<?php echo base_url('saisie/Groupes/add')?>";
  }
  else
  {
    url="<?php echo base_url('saisie/Groupes/update')?>";
  }
...
function edit_groupe(id)
{
  ...
  $('#u_form')[0].reset();
  $.ajax({
    url : "<?php echo site_url('saisie/Groupes/getOne')?>/" + id,
    type: "GET",
    dataType: "JSON",
    success: function(data)
    {
      ...
      $('[name="GROUPE_DESC"]').val(data.GROUPE_DESC);
      $('#u-modal').modal('show'); 
...
function supp_logic(id,is_actif)
{
  ...
  if (is_actif==1) {message="Voulez-vous désactiver?"} else {message="Voulez-vous activer?"}
   if (confirm(message)) {
    $.ajax({
      url : "<?php echo base_url('saisie/Groupes/supp_logic')?>/"+id+'/'+is_actif,
      type: "POST",
      dataType: "JSON",
      success: function(data)
      {
                  ...
                  <div class="col-md-12">
                    <div class="form-group">
                      <div class="mb-3">
                        ...
                        <textarea class="form-control" name="GROUPE_DESC" id="GROUPE_DESC" rows="2" autofocus data-provide="typeahead"  autocomplete="off" autofocus></textarea>
