...
class Magasin_sales extends CI_Controller
{
  ...
  function index()
  {
    ...
    $data['CLIENT'] = $this->Model->getRequete("SELECT GROS_CLIENT_ID,RS,NIF,ADRESS_GROS_CLIENT,IS_CLIENT,ID_BRANCHE,NOM_GROS_CLIENT,TEL_GROS_CLIENT FROM `gros_client` WHERE IS_CLIENT=1 AND ID_BRANCHE=".$this->session->userdata('ID_BRANCHE')) ;
    $this->cart->destroy();
  ...
  {
    $data_stock = $this->Model->getOne('products', array('PRODUCT_ID' => $id));
    echo json_encode($data_stock);
  }
  ...
  public function add_in_cart()
  {
    ...
    foreach ($this->cart->contents() as $item) :
      if (preg_match('/FILE/', $item['typecartitem'])) {
        ...
        <a  class="action-icon text-danger"> <i class="mdi mdi-delete" onclick="remove_car(' . $j . ')">
        </a></td>
        </tr>';
...
{ 
  if ($value['UNITE_MESURE']=1) {
  $DES=$this->Model->getRequeteOne('SELECT * FROM products WHERE PRODUCT_ID='.$value['DESIGNATION']);
  }
  else{
    $DES=$this->Model->getRequeteOne('SELECT * FROM stock_secretariat  WHERE TYPE_STOCK="'.$value['type_produit'].'" AND PRODUCT_ID='.$value['DESIGNATION']);
  }
  $PRODUCT_ID=$DES['SECR_STOCK_ID'];
  $P_ACHAT_TOTAL=0;
  $PRIX_TOTAL=0;
  if ($value['UNITE_MESURE']!=1) {
  ...
  $OLD=$this->Model->getRequeteOne('SELECT * FROM stock_secretariat WHERE PRODUCT_ID='.$value['DESIGNATION'].' AND TYPE_STOCK='.$value['TYPE_STOCK']);
  $OLD=$this->Model->getRequeteOne('SELECT * FROM stock_secretariat WHERE SECR_STOCK_ID='.$PRODUCT_ID);
  ...
  //   $success=$this->Model->update('stock_secretariat',array('PRODUCT_ID'=>$value['DESIGNATION'],'TYPE_STOCK'=>$value['TYPE_STOCK']),$arrayds);
  // }
  ...
  $facture=$this->Model->getRequete('SELECT * FROM mag_client_facture join mag_ventes_produits ON mag_ventes_produits.GROS_CLIENT_FACT_ID=mag_client_facture.GROS_CLIENT_FACT_ID JOIN stock_secretariat ON stock_secretariat.SECR_STOCK_ID=mag_ventes_produits.PRODUCT_ID join products ON products.PRODUCT_ID=stock_secretariat.PRODUCT_ID WHERE mag_client_facture.GROS_CLIENT_FACT_ID='.$GROS_CLIENT_FACT_ID);
 ...
 if (empty($client['TEL_GROS_CLIENT'])) {
  ...
  $pdf->Cell(40,6, 'S A N Y A  S H O P  B U R U N D I', 0, 1);
  $pdf->Cell(45 ,3,'',0,0);
  $pdf->SetFont('times', 'BI', 10);
  $pdf->Cell(100, 5, 'M  a  r  c  h  e    d e   b  i  e  n  s   c  u  l  t  u  r  e  l  s', 0, 1);
  $pdf->Cell(48 ,5,'',0,0);
