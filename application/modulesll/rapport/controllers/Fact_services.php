...
class Fact_services extends CI_Controller
{
  ...
  $facture=$this->Model->getRequete('SELECT * FROM mag_client_factur join mag_ventes_services ON mag_ventes_services.GROS_CLIENT_FACT_ID=mag_client_factur.GROS_CLIENT_FACT_ID JOIN stock_secretariat ON stock_secretariat.SECR_STOCK_ID=mag_ventes_services.STOCK_ID join sanya_services ON sanya_services.PRODUCT_ID=stock_secretariat.PRODUCT_ID WHERE 1 AND  mag_client_factur.GROS_CLIENT_FACT_ID='.$GROS_CLIENT_FACT_ID);
