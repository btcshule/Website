...
class Dettes_internes extends CI_Controller
{
	...
	function index()
	{
		$data['page_title']="Gestionnaire des Dettes Internes";
		...
		$this->load->view('Dettes_internes_view',$data);
	}
	...
	function liste()
	{
		...
		$query_principal="SELECT `ID_DETTE_INTERNE`,NOM_FOURNISSEUR,PRENOM_FOURNISSEUR, `DESIGNATION`, `MONTANT`, `DATE_DETTE`, dettes_internes.DATE_INSERTION, dettes_internes.STATUT FROM `dettes_internes` LEFT JOIN fournisseurs_sanya ON fournisseurs_sanya.ID_FOURNISSEUR=dettes_internes.ID_FOURNISSEUR WHERE 1";
		...
		foreach ($fetch_data as $row) {
			...
			$sub_array[] = '<a href="javascript:void(0);" style="color:green;" onclick="edit_emp('.$row->ID_DETTE_INTERNE.')" class="action-icon"> <i class="mdi mdi-square-edit-outline"></i></a>
			<a href="javascript:void(0);" onclick="supp_logic('.$row->ID_DETTE_INTERNE.','.$row->STATUT.')" style="color:red;" class="action-icon"> <i class="mdi mdi-delete"></i></a>';
