...
class Lumicash extends CI_Controller
{
  ...
  function ajouter()
  {
    ...
    // SELECT `ID_AGENT`, `TYPE_AGENT`, `CASH`, `ELECTRONIQUE`, `DESC_TRANS`, `DEBIT`, `CREDIT`, `SOLDE`, `EST_ENTREE`, `DATE_INSERTION`, `RESP_ELECT` FROM `electroniques` WHERE 1
    $data=array('CASH'=>trim($CASH),'DESC_TRANS'=>trim($COMMENTS),'ELECTRONIQUE'=>trim($ELECTRONIQUE), 'TYPE_AGENT'=>trim($TYPE_AGENT),'DEBIT'=>trim($SOMME_DEBIT),'CREDIT'=>trim($SOMME_CREDIT),'SOLDE'=>trim($SOLDE1),'EST_ENTREE'=>trim($EST_ENTREE),'RESP_ELECT'=>trim($RESP_ELECT));
     // SELECT `ID_ELE`, `LIBELLE`, `IMPUTATION`, `DEBIT`, `CREDIT`, `SOLDE`, `EST_ENTREE`, `STATUT`, `DATE_ACTION`, `RESP_ELECT` FROM `data_electronique` WHERE 1
  ...
  function liste()
  {
    ...
    $query_principal="SELECT `ID_AGENT`,  `TYPE_AGENT`,`CASH`, `ELECTRONIQUE`, `DESC_TRANS`, `DATE_INSERTION`, `RESP_ELECT`,PRENOM_EMP,NOM_EMP, EST_ENTREE,SOLDE FROM `electroniques`  JOIN employes ON employes.EMPLOYE_ID=electroniques.RESP_ELECT WHERE 1 AND EST_ENTREE=1";
