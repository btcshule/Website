...
<script>
  ...
  $(document).ready(function() {
   ...
   table=$("#mytable").DataTable({
    ...
    "order":[],
    "ajax":{
      url:"<?php echo base_url('index.php/suivi/Comptes/liste')?>",
      type:"POST"
    },
...
<script>
  function enr()
  {
    var DESC_COMPTE=$('#DESC_COMPTE').val();
    var NO_COMPTE=$('#NO_COMPTE').val();

    var statut = true;
    if (DESC_COMPTE=="") {
      $("#errDESC_COMPTE").html("Le champ est obligatoire");
      statut=false;
    }else{
      $("#errDESC_COMPTE").html("");
    }
    ...
    if (NO_COMPTE=="") {
      $("#errNO_COMPTE").html("Le champ est obligatoire");
      statut=false;
    }else{
      $("#errNO_COMPTE").html("");
    }
    if (statut==true) {
      ...
      if (ID_COMPTE=="") 
      {
        ...
        url="<?php echo base_url('index.php/suivi/Comptes/ajouter')?>";
      }
      else
      {
        url="<?php echo base_url('index.php/suivi/Comptes/update')?>";
      }
  ...
  function edit_emp(id)
{
 ...
 $('#emp_form')[0].reset();
    $('#emp-modal').modal('show');
  $.ajax({
    url : "<?php echo site_url('index.php/suivi/Comptes/getOne')?>/" + id,
    type: "GET",
    dataType: "JSON",
    success: function(data)
    {
      ...
      $('[name="NO_COMPTE"]').val(data.NO_COMPTE);
      $('[name="DESC_COMPTE"]').val(data.DESC_COMPTE); 
      $('[name="ID_COMPTE"]').val(data.ID_COMPTE); 
  ...
  function supp_logic(id,is_actif)
  {
    ...
    if (is_actif==1) {message="Voulez-vous désactiver?"} else {message="Cette action n'est pas autorisée"}
     if (confirm(message)) {
      $.ajax({
        url : "<?php echo base_url('index.php/suivi/Comptes/del')?>/"+id+'/'+is_actif,
        type: "POST",
        dataType: "JSON",
        success: function(data)
        {
                ...
                <div class="row">
                    ...
                    <div class="form-group">
                      <div class="mb-3">
                        ...
                        <input type="number" autocomplete="off" name="NO_COMPTE" class="form-control" id="NO_COMPTE">
                        <span class="text-danger" id="errNO_COMPTE"></span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      <div class="mb-3">
                        ...
                        <input type="text" autocomplete="off" name="DESC_COMPTE" class="form-control" id="DESC_COMPTE" autofocus>
                        <span class="text-danger" id="errDESC_COMPTE"></span>
                      </div>
                    </div>
                  </div>
