...
<script>
	...
	$(document).ready(function() {
		...
		table=$("#mytable").DataTable({
			...
			"order":[],
			"ajax":{
				url:"<?php echo base_url('index.php/suivi/Banque_retrait/liste')?>",
				type:"POST"
			},
...
<script>
	function enr()
	{
		...
		if (statut==true) {
			...
			if (ID_LIVRE_CAISSE=="") 
			{
				...
				url="<?php echo base_url('index.php/suivi/Banque_retrait/ajouter')?>";
			}
			else
			{
				url="<?php echo base_url('index.php/suivi/Banque_retrait/update')?>";
			}
	...
	function edit_emp(id)
	{
		...
		$('#emp-modal').modal('show');
		$.ajax({
			url : "<?php echo site_url('index.php/suivi/Banque/getOne')?>/" + id,
			type: "GET",
			dataType: "JSON",
			success: function(data)
			{
				...
				$('[name="ID_LIVRE_BANQUE"]').val(data.ID_LIVRE_BANQUE); 
	...
	function supp_logic(id,is_actif)
	{
		...
		if (is_actif==1) {message="Voulez-vous désactiver?"} else {message="Cette action n'est pas autorisée"}
			if (confirm(message)) {
				$.ajax({
					url : "<?php echo base_url('index.php/suivi/Banque/del')?>/"+id+'/'+is_actif,
					type: "POST",
					dataType: "JSON",
					success: function(data)
					{
											...
											<div class="mb-3">
												...
												foreach ($imputation as  $value) {?>
													<option value="<?=  $value['ID_IMPUTATION'] ?>">
														(<?= $value['CODE_IMPUTATION'] ?>) <?= $value['DESC_IMPUTATION'] ?></option>
													<?php  }  ?>
										...
										<div class="col-md-6">
											<div class="form-group">
												<div class="mb-3">
													...
													<input type="text" autocomplete="off" name="MONTANT" class="form-control" id="MONTANT">
