...
<script>
	...
	$(document).ready(function() {
		...
		table=$("#mytable").DataTable({
			...
			"order":[],
			"ajax":{
				url:"<?php echo base_url('index.php/suivi/Sorties/liste')?>",
				type:"POST"
			},
...
<script>
	function enr()
	{
		...
		if (statut==true) {
			...
			if (ID_LIVRE_CAISSE=="") 
			{
				...
				url="<?php echo base_url('index.php/suivi/Sorties/ajouter')?>";
			}
			else
			{
				url="<?php echo base_url('index.php/suivi/Sorties/update')?>";
			}
	...
	function edit_emp(id)
	{
		...
		$('#emp-modal').modal('show');
		$.ajax({
			url : "<?php echo site_url('index.php/suivi/Sorties/getOne')?>/" + id,
			type: "GET",
			dataType: "JSON",
			success: function(data)
			{
	...
	function supp_logic(id, is_actif) {
		...
		if (confirm(message)) {
			$.ajax({
				url: "<?php echo base_url('index.php/suivi/Sorties/del')?>/" + id + '/' + is_actif,
				type: "POST",
				dataType: "JSON",
				success: function(data) {
					if (data.status) {
