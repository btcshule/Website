...
class Ventes extends CI_Controller
{
  ...
  function index()
  {
    ...
    $data['page_title']="Gestion des ventes";
...
$formattedDate = $dateTime->format('Y-m-d H:i:s');
  ...
  function liste()
  {
    ...
    $query_principal="SELECT `ID_VENTE`, NOM_CLIENT,PRENOM_CLIENT, `PRODUCT_DESC`, `QUANTITE`, `PU`, `PT`, `OBSERVATION`, NOM_EMP,PRENOM_EMP, `DATE_VENTE`, ventes_sanya.DATE_CREATION, ventes_sanya.STATUT FROM `ventes_sanya` LEFT JOIN employes ON employes.EMPLOYE_ID=ventes_sanya.RESP_VENTE LEFT JOIN clients_sanya ON clients_sanya.ID_CLIENT=ventes_sanya.ID_CLIENT LEFT JOIN products ON products.PRODUCT_ID=ventes_sanya.DESIGNATION WHERE ventes_sanya.STATUT=1";
    $order_column=array('NOM_CLIENT','PRENOM_CLIENT','PRODUCT_DESC','QUANTITE','OBSERVATION','DATE_VENTE','NOM_EMP','PRENOM_EMP');

    $order_by = isset($_POST['order']) ? ' ORDER BY '.$order_column[$_POST['order']['0']['column']] .'  '.$_POST['order']['0']['dir'] : 'ORDER  BY DATE_VENTE  DESC';
    $search = !empty($_POST['search']['value']) ? (" AND  (PRODUCT_DESC LIKE '%$var_search%' OR NOM_CLIENT LIKE '%$var_search%' OR DATE_VENTE LIKE '%$var_search%' OR ventes_sanya.DATE_CREATION LIKE '%$var_search%' OR OBSERVATION LIKE '%$var_search%') ") : '';
    $limit='LIMIT 0,10';
    if($_POST['length'] != -1){
