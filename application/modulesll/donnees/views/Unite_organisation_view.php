...
<script>
  ...
  $(document).ready(function() {
   ...
   table=$("#mytable").DataTable({
    ...
    "order":[],
    "ajax":{
      url:"<?php echo base_url('donnees/Unite_organisation/liste')?>",
      type:"POST"
    },
...
<script>
  function enr()
  {
    var OU_DESC=$('#OU_DESC').val();
    var CODE=$('#CODE').val();
    var statut = true;
    if (OU_DESC=="") {
      $("#errOU_DESC").html("Le champ est obligatoire");
      statut=false;
    }else{
      $("#errIMMOB_DESC").html("");
    }
    if (CODE=="") {
      $("#errCODE").html("Le champ est obligatoire");
      statut=false;
    }else{
      $("#errCODE").html("");
    }
    if (statut==true) {
      ...
      if (OU_ID=="") 
      {
        ...
        url="<?php echo base_url('donnees/Unite_organisation/add')?>";
      }
      else
      {
        url="<?php echo base_url('donnees/Unite_organisation/update')?>";
      }
  ...
  function edit_emp(id)
{
 ...
 $('#emp_form')[0].reset();
    $('#emp-modal').modal('show');
  $.ajax({
    url : "<?php echo site_url('donnees/Unite_organisation/getOne')?>/" + id,
    type: "GET",
    dataType: "JSON",
    success: function(data)
    {
      ...
      $('[name="CODE"]').val(data.OU_NUM);
      $('[name="OU_DESC"]').val(data.OU_DESC); 
      $('[name="OU_ID"]').val(data.ID_OU); 
  ...
  function supp_logic(id,is_actif)
  {
    ...
    if (is_actif==1) {message="Voulez-vous désactiver?"} else {message="Voulez-vous activer?"}
     if (confirm(message)) {
      $.ajax({
        url : "<?php echo base_url('donnees/Unite_organisation/del')?>/"+id+'/'+is_actif,
        type: "POST",
        dataType: "JSON",
        success: function(data)
        {
        ...
        <div class="row">
          <div class="col-12">
            <form id="emp_form" method="POST">
              <div class="form-body">
                <input type="hidden" name="OU_ID" id="OU_ID">
                <div class="row">
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      <div class="mb-3">
                        ...
                        <input type="text" autocomplete="off" name="OU_DESC" class="form-control" id="OU_DESC" autofocus>
                        <span class="text-danger" id="errOU_DESC"></span>
                      </div>
                    </div>
                  </div>
                   <div class="col-md-6">
                    <div class="form-group">
                      <div class="mb-3">
                        ...
                        <input type="text" autocomplete="off" name="CODE" class="form-control" id="CODE" autofocus>
