...
<script>
  ...
  $(document).ready(function() {
   ...
   table=$("#mytable").DataTable({
    ...
    "order":[],
    "ajax":{
      url:"<?php echo base_url('donnees/Occupation/liste')?>",
      type:"POST"
    },
  ...
  function enr()
  {
    ...
    if (statut==true) {
      ...
      if (OCCUPATION_ID=="") 
      {
        ...
        url="<?php echo base_url('donnees/Occupation/add')?>";
      }
      else
      {
        url="<?php echo base_url('donnees/Occupation/update')?>";
      }
 ...
 $('#emp_form')[0].reset();
  ...
  function supp_logic(id,is_actif)
  {
    ...
    if (is_actif==1) {message="Voulez-vous désactiver?"} else {message="Voulez-vous activer?"}
     if (confirm(message)) {
      $.ajax({
        url : "<?php echo base_url('donnees/Occupation/del')?>/"+id+'/'+is_actif,
        type: "POST",
        dataType: "JSON",
        success: function(data)
        {
