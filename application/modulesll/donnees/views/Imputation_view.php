...
<script>
  ...
  $(document).ready(function() {
   ...
   table=$("#mytable").DataTable({
    ...
    "order":[],
    "ajax":{
      url:"<?php echo base_url('index.php/donnees/Imputation/liste')?>",
      type:"POST"
    },
...
<script>
  function enr()
  {
    var CODE_IMPUTATION=$('#CODE_IMPUTATION').val();
    var DESC_IMPUTATION=$('#DESC_IMPUTATION').val();

    var statut = true;
    if (CODE_IMPUTATION=="") {
      $("#errCODE_IMPUTATION").html("Le champ est obligatoire");
      statut=false;
    }else{
      $("#errCODE_IMPUTATION").html("");
    }

    if (DESC_IMPUTATION=="") {
      $("#errDESC_IMPUTATION").html("Le champ est obligatoire");
      statut=false;
    }else{
      $("#errDESC_IMPUTATION").html("");
    }
    if (statut==true) {
      ...
      var url;
      var ID_IMPUTATION=$('#ID_IMPUTATION').val();
      
      if (ID_IMPUTATION=="") 
      {

        url="<?php echo base_url('index.php/donnees/Imputation/ajouter')?>";
      }
      else
      {
        url="<?php echo base_url('index.php/donnees/Imputation/update')?>";
      }
  ...
  function edit_emp(id)
{
 ...
 $('#emp_form')[0].reset();
    $('#emp-modal').modal('show');
  $.ajax({
    url : "<?php echo site_url('index.php/donnees/Imputation/getOne')?>/" + id,
    type: "GET",
    dataType: "JSON",
    success: function(data)
    {
      ...
      $('[name="CODE_IMPUTATION"]').val(data.CODE_IMPUTATION);
      $('[name="DESC_IMPUTATION"]').val(data.DESC_IMPUTATION); 
      $('[name="ID_IMPUTATION"]').val(data.ID_IMPUTATION); 
  ...
  function supp_logic(id,is_actif)
  {
    ...
    if (is_actif==1) {message="Voulez-vous désactiver?"} else {message="Voulez-vous activer?"}
     if (confirm(message)) {
      $.ajax({
        url : "<?php echo base_url('index.php/donnees/Imputation/del')?>/"+id+'/'+is_actif,
        type: "POST",
        dataType: "JSON",
        success: function(data)
        {
        ...
        <div class="row">
          <div class="col-12">
            <form id="emp_form" method="POST">
              <div class="form-body">
                <input type="hidden" name="ID_IMPUTATION" id="ID_IMPUTATION">
                <div class="row">
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      <div class="mb-3">
                        ...
                        <input type="text" autocomplete="off" name="CODE_IMPUTATION" class="form-control" id="CODE_IMPUTATION" autofocus>
                        <span class="text-danger" id="errCODE_IMPUTATION"></span>
                      </div>
                    </div>
                  </div>
                    ...
                    <div class="form-group">
                      <div class="mb-3">
                        ...
                        <input type="text" autocomplete="off" name="DESC_IMPUTATION" class="form-control" id="DESC_IMPUTATION">
                        <span class="text-danger" id="errDESC_IMPUTATION"></span>
                      </div>
                    </div>
                  </div>
