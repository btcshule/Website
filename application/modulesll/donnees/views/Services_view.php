...
<script>
  ...
  $(document).ready(function() {
   ...
   table=$("#mytable").DataTable({
    ...
    "order":[],
    "ajax":{
      url:"<?php echo base_url('index.php/donnees/Services/liste')?>",
      type:"POST"
    },
...
<script>
  function enr()
  {
    ...
    if (PRIX=="") {
      $("#errPRIX").html("Le champ est obligatoire");
      statut=false;
    }else{
      $("#errPRIX").html("");
    }
    if (statut==true) {
      ...
      if (ID_SERVICE=="") 
      {
        ...
        url="<?php echo base_url('index.php/donnees/Services/ajouter')?>";
      }
      else
      {
        url="<?php echo base_url('index.php/donnees/Services/update')?>";
      }
  ...
  function edit_emp(id)
{
 ...
 $('#emp_form')[0].reset();
    $('#emp-modal').modal('show');
  $.ajax({
    url : "<?php echo site_url('index.php/donnees/Services/getOne')?>/" + id,
    type: "GET",
    dataType: "JSON",
    success: function(data)
    {
      ...
      $('[name="PRIX"]').val(data.PU);
      $('[name="DESIGNATION"]').val(data.DESIGNATION); 
      $('[name="ID_SERVICE"]').val(data.ID_SERVICE); 
  ...
  function supp_logic(id,is_actif)
  {
    ...
    if (is_actif==1) {message="Voulez-vous désactiver?"} else {message="Voulez-vous activer?"}
     if (confirm(message)) {
      $.ajax({
        url : "<?php echo base_url('index.php/donnees/Services/del')?>/"+id+'/'+is_actif,
        type: "POST",
        dataType: "JSON",
        success: function(data)
        {
                  ...
                  <div class="col-md-6">
                    ...
                    <div class="form-group">
                      <div class="mb-3">
                        ...
                        <input type="text" autocomplete="off" name="PRIX" class="form-control" id="PRIX">
                        <span class="text-danger" id="errPRIX"></span>
                      </div>
                    </div>
                  </div>
