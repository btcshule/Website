...
<script>
  ...
  $(document).ready(function() {
   ...
   table=$("#mytable").DataTable({
    ...
    "order":[],
    "ajax":{
      url:"<?php echo base_url('donnees/Immobilier/liste')?>",
      type:"POST"
    },
...
<script>
  function enr()
  {
    var IMMOB_DESC=$('#IMMOB_DESC').val();
    var TYPE_IMMO_ID=$('#TYPE_IMMO_ID').val();
    var statut = true;
    if (IMMOB_DESC=="") {
      $("#errIMMOB_DESC").html("Le champ est obligatoire");
      statut=false;
    }else{
      $("#errIMMOB_DESC").html("");
    }
    if (TYPE_IMMO_ID=="") {
      $("#errTYPE_IMMO_ID").html("Le champ est obligatoire");
      statut=false;
    }else{
      $("#errTYPE_IMMO_ID").html("");
    }
    if (statut==true) {
      ...
      var url;
      var ID_IMMOBILIER=$('#ID_IMMOBILIER').val();
      
      if (ID_IMMOBILIER=="") 
      {

        url="<?php echo base_url('donnees/Immobilier/add')?>";
      }
      else
      {
        url="<?php echo base_url('donnees/Immobilier/update')?>";
      }
  ...
  function edit_emp(id)
{
 ...
 $('#emp_form')[0].reset();
    $('#emp-modal').modal('show');
  $.ajax({
    url : "<?php echo site_url('donnees/Immobilier/getOne')?>/" + id,
    type: "GET",
    dataType: "JSON",
    success: function(data)
    {
      ...
      $('[name="TYPE_IMMO_ID"]').val(data.TYPE_IMMO_ID);
      $('[name="IMMOB_DESC"]').val(data.IMMOB_DESC); 
      $('[name="ID_IMMOBILIER"]').val(data.ID_IMMOBILIER); 
  ...
  function supp_logic(id,is_actif)
  {
    ...
    if (is_actif==1) {message="Voulez-vous désactiver?"} else {message="Voulez-vous activer?"}
     if (confirm(message)) {
      $.ajax({
        url : "<?php echo base_url('donnees/Immobilier/del')?>/"+id+'/'+is_actif,
        type: "POST",
        dataType: "JSON",
        success: function(data)
        {
        ...
        <div class="row">
          <div class="col-12">
            <form id="emp_form" method="POST">
              <div class="form-body">
                <input type="hidden" name="ID_IMMOBILIER" id="ID_IMMOBILIER">
                <div class="row">
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      <div class="mb-3">
                        ...
                        <input type="text" autocomplete="off" name="IMMOB_DESC" class="form-control" id="IMMOB_DESC" autofocus>
                        <span class="text-danger" id="errIMMOB_DESC"></span>
                      </div>
                    </div>
                  </div>
                  <div class="col-md-6">
                    <div class="form-group">
                      <div class="mb-3">
                        ...
                        <select class="form-control" id="TYPE_IMMO_ID" onchange="get_ou()" name="TYPE_IMMO_ID">
                          <option value="">Sélectionner</option>
                          <?php
                          foreach ($type as $a) {
                                 
                            echo "<option value=".$a['TYPE_IMMO_ID'].">".$a['TYPE_IMMO_DESC']."</option>";
                          }
                          ?>

                        </select>
                        <span class="text-danger" id="errTYPE_IMMO_ID"></span>
                      </div>
                    </div>
                  </div>
                </div>
