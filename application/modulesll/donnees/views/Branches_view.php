...
<script>
  ...
  $(document).ready(function() {
   ...
   table=$("#mytable").DataTable({
    ...
    "order":[],
    "ajax":{
      url:"<?php echo base_url('index.php/donnees/Branches/liste')?>",
      type:"POST"
    },
...
<script>
  function enr()
  {
    var DESCRIPTION_BRANCH=$('#DESCRIPTION_BRANCH').val();
    var LOCALISATION=$('#LOCALISATION').val();

    var statut = true;
    if (DESCRIPTION_BRANCH=="") {
      $("#errDESCRIPTION_BRANCH").html("Le champ est obligatoire");
      statut=false;
    }else{
      $("#errDESCRIPTION_BRANCH").html("");
    }
    ...
    if (statut==true) {
      ...
      if (ID_BRANCHE=="") 
      {
        ...
        url="<?php echo base_url('index.php/donnees/Branches/ajouter')?>";
      }
      else
      {
        url="<?php echo base_url('index.php/donnees/Branches/update')?>";
      }
  ...
  function edit_emp(id)
{
 ...
 $('#emp_form')[0].reset();
    $('#emp-modal').modal('show');
  $.ajax({
    url : "<?php echo site_url('index.php/donnees/Branches/getOne')?>/" + id,
    type: "GET",
    dataType: "JSON",
    success: function(data)
    {
      ...
      $('[name="LOCALISATION"]').val(data.LOCALISATION);
      $('[name="DESCRIPTION_BRANCH"]').val(data.DESCRIPTION_BRANCH); 
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      <div class="mb-3">
                        ...
                        <input type="text" autocomplete="off" name="DESCRIPTION_BRANCH" class="form-control" id="DESCRIPTION_BRANCH" autofocus>
                        <span class="text-danger" id="errDESCRIPTION_BRANCH"></span>
                      </div>
                    </div>
                  </div>
