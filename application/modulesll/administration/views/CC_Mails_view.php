...
<script>
  ...
  $(document).ready(function() {
   ...
   table=$("#mytable").DataTable({
    ...
    "order":[],
    "ajax":{
      url:"<?php echo base_url('administration/CC_Mails/liste')?>",
      type:"POST"
    },
...
<script>
  ...
  function enr()
  {
   ...
   if (save_method=="add") 
   {
    url="<?php echo base_url('administration/CC_Mails/add')?>";
  }
  else
  {
    url="<?php echo base_url('administration/CC_Mails/update')?>";
  }
...
function edit_cc(id)
{
  ...
  $('#u_form')[0].reset();
  $.ajax({
    url : "<?php echo site_url('administration/CC_Mails/getOne')?>/" + id,
    type: "GET",
    dataType: "JSON",
    success: function(data)
    {

      $('[name="CC_MAIL_ID"]').val(data.CC_MAIL_ID);
      $('[name="CC_MAIL_DESC"]').val(data.CC_MAIL_DESC);
      $('#u-modal').modal('show'); 
...
function supp_logic(id,is_actif)
{
  ...
  if (is_actif==1) {message="Voulez-vous désactiver?"} else {message="Voulez-vous activer?"}
   if (confirm(message)) {
    $.ajax({
      url : "<?php echo base_url('administration/CC_Mails/supp_logic')?>/"+id+'/'+is_actif,
      type: "POST",
      dataType: "JSON",
      success: function(data)
      {
        ...
        <div class="row">
          <div class="col-12">
            <form id="u_form" method="POST">
              <div class="form-body">
                <input type="hidden" name="CC_MAIL_ID" id="CC_MAIL_ID">
                <div class="row">
                  <div class="col-md-12">
                    <div class="form-group">
                      <div class="mb-3">
                        <label class="form-label"> CC<span style="color:red;">*</span></label>
                        <input type="text" class="form-control" autocomplete="off" name="CC_MAIL_DESC" id="CC_MAIL_DESC">
